...
// currently defines a default body for, as of alloy-evm 0.34.0 (streamingfast/evm.git branch
// `sf/v0.34.0`):
//   execute_transaction, execute_transaction_with_result_closure,
//   execute_transaction_with_commit_condition, apply_post_execution_changes, with_state_hook,
//   execute_block.
// Of these, `execute_transaction_with_commit_condition` is the sole point chain integrators
// (e.g. an Arc or OP Stack producer wrapped as `Inner`) hook to change per-tx commit/decline
// behavior, and `execute_transaction` / `execute_transaction_with_result_closure` both bottom out
// in it via the trait's own default bodies.
// `apply_post_execution_changes`, `execute_block` and `with_state_hook` are deliberately NOT
// forwarded to `self.inner` — their defaults route back through
// `Self::finish`/`Self::apply_pre_execution_changes`/`Self::execute_transaction`/
// `Self::set_state_hook`, i.e. through *our* overrides; forwarding them to `self.inner` instead
// would bypass this wrapper entirely.
//
...
// delegation upstream was reverted because it is not achievable without breaking tracing
// correctness — see the long comment inside the method body for why (it is a proven, not
// theoretical, regression: caught by golden files, every `RewardTransactionFee.old_value` came out
// wrong). Practical effect: if a wrapped `Inner` overrides
// `execute_transaction_with_commit_condition` (e.g. a refund-policy snapshot/restore around a
// *declined* candidate — `CommitChanges::No`), that override does not run through this wrapper.
// This does not affect canonical/traced block execution, which always commits
// (`CommitChanges::Yes`) via the `execute_transaction`/`execute_block` convenience path; it only
// matters for callers that invoke `execute_transaction_with_commit_condition` directly with a real
// decline condition (e.g. speculative/candidate execution during block building).
...
// to whoever owns the producer/candidate-execution path if the decline branch is reachable with
// Firehose tracing enabled.
//
    ...
    Adjust: PreTxAdjust<Inner::Evm>,
{
    ...
    fn execute_transaction_with_commit_condition(
        ...
        // and chain integrators can override it — e.g. to snapshot and restore refund-policy state
        // around a declined candidate. Delegating this whole method to
        // `self.inner.execute_transaction_with_commit_condition` was tried upstream and reverted:
        // the closure passed to it cannot re-borrow `self.inner` (already held by the outer call),
        // so all EVM/inspector-dependent accounting below would have to move to *after* that call
        // returns. `process_post_tx_balance_changes`'s `get_pre_tx_balance` fallback is the
        // *primary* source of the coinbase `RewardTransactionFee` `old_value` (gas-refund and
        // coinbase-tip crediting produce no `JournalEntry`), and it must read pre-commit DB
        ...
        // `old_value` (verified against golden files: every `RewardTransactionFee` reported the
        // post-reward balance as its own pre-reward baseline).
        // `PostTxExtras::emit_post_tx_extras` has the identical requirement for chain-specific
        // addresses this crate cannot enumerate, so pre-capturing a fixed address set does not
        // generalize either. Splitting into `execute_transaction_without_commit` +
        // `commit_transaction` remains the only safe option until alloy-evm's
        // `execute_transaction_with_commit_condition` closure signature is changed (in the
        // streamingfast/evm.git fork) to also hand the closure `&mut Self::Evm`.
        // KNOWN GAP: because of this, an `Inner` override of
        // `execute_transaction_with_commit_condition` does not run through this wrapper — see the
        // GUARD comment above `impl BlockExecutor for FirehoseWrappedExecutor` for what that means
        // in practice.
