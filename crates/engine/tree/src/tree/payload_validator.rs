    ...
    Evm: ConfigureEvm<Primitives = N> + 'static,
{
        ...
        TxTy<N>: reth_firehose::mapper::SignatureFields,
    {
        ...
        // Firehose tracing executes transactions sequentially in block order, so the parallel BAL
        // path (which streams transactions out of order) must stay off while the tracer is active.
        let firehose_tracing = reth_firehose::is_tracer_initialized();
        let parallel_bal_execution =
            ensure_ok!(self.bal_path_eligible(env.decoded_bal.as_deref())) && !firehose_tracing;
    ...
    ///
    /// JIT support is never enabled here: JIT-compiled code does not drive the inspector hooks the
    /// tracer depends on.
    ///
